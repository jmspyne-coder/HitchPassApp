...
for region,states in TT.items():
    for st,items in states.items():
        for (name,city) in items:
            pid+=1; parks.append({"id":pid,"name":name,"city":city,"st":st,"region":region,"network":"tt"})
tt_count=pid
for st,names in TC.items():
    for name in names:
        pid+=1; parks.append({"id":pid,"name":name,"city":"","st":st,"region":"Trails Collection","network":"enc"})
enc_count=pid-tt_count
assert tt_count==82 and enc_count==122 and len(parks)==204

def q(s): return json.dumps(s, ensure_ascii=False)
def row(p):
    return '    { id:%-4s name:%s, city:%s, st:%s, region:%s, network:%s }' % (
        str(p["id"])+",", q(p["name"]), q(p["city"]), q(p["st"]), q(p["region"]), q(p["network"]))
body = "  var PARKS = [\n" + ",\n".join(row(p) for p in parks) + "\n  ];"

path = r"C:\Users\jmspy\HitchPassApp\index.html"
with open(path, "r", encoding="utf-8") as f:
    html = f.read()
pat = re.compile(r"  var PARKS = \[.*?\n  \];", re.DOTALL)
m = pat.search(html)
assert m, "PARKS array not found"
old = m.group(0)
assert old.count("id:") == 24, "expected 24 sample parks, found %d" % old.count("id:")
html2 = html[:m.start()] + body + html[m.end():]
with open(path, "w", encoding="utf-8", newline="\n") as f:
    f.write(html2)
print("Replaced %d sample parks with %d real parks (tt=%d enc=%d)" % (24, len(parks), tt_count, enc_count))
print("New PARKS block id count:", body.count("id:"))
